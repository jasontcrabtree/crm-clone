...
public class InteractionController : BaseController
{
    ...
    public async Task<IActionResult> GetAll([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 20)
    {
        ...
        var (interactions, totalCount) = await _interactionService.GetAllInteractions(userId, pageNumber, pageSize);
        return Ok(new { data = interactions, total = totalCount });
    }
    ...
    public async Task<IActionResult> GetInteractionsByEntityType(string entityType)
    {
        int userId = GetUserId();
        var interactions = await _interactionService.GetUserInteractionsForEntityTypeAsync(entityType, userId);
        return Ok(interactions);
    }
