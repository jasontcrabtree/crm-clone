...
const CorePageLayout = ({ children }: { children: React.ReactNode }) => {
    return (
        <div className="flex w-full bg-white flex-col-reverse sm:flex-row">
            {children}
        </div>
    )
}
    ...
    return (
        <CorePageLayout>
            <Suspense fallback={<DashboardSkeleton />}>
                {children}
                <div className="bg-teal-800 text-white w-full sm:min-w-24 sm:max-w-48 sm:h-screen ml-auto">
                    Action slot
                </div>
            </Suspense>
        </CorePageLayout>
